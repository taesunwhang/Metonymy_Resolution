...
def sort_by_seq_lens(batch, sequence_lengths, descending=True):
    ...
    Args:
        ...
        sequence_lengths: A tensor containing the lengths of the sequences in the
    ...
    """
    # 0, 2, 3, 1, 4
    sorted_seq_lens, sorting_index = sequence_lengths.sort(0, descending=descending) # torch.sort : dim
    sorted_batch = batch.index_select(0, sorting_index)
    idx_range = sequence_lengths.new_tensor(torch.arange(0, len(sequence_lengths))) # 0, 1, 2, ..., len(seq_lengths)
    # 0, 3, 1, 3, 4
...
def get_mask(sequences_batch, sequences_lengths, pad_idx=0):
    """
    ...
    mask[sequences_batch[:, :max_length] == pad_idx] = 0.0
    return mask
...
def weighted_cross_entropy_loss(class_dist):

    weights = [sum(class_dist) / n for n in class_dist]
